...
class MensajeApp:
    ...
    def __init__(self, master):
        ...
        tk.Button(master, text="Eliminar Mensaje", command=self.eliminar_mensaje).grid(row=6, column=0, columnspan=2)
        tk.Button(master, text="Responder Mensaje", command=self.responder_mensaje).grid(row=7, column=0, columnspan=2, pady=10)
    ...
    def responder_mensaje(self):
        self.conectar_base_datos()
        id_mensaje = self.id_var.get()
        gestion = "Respuesta de prueba"  # Puedes cambiar esto según tus necesidades
        ...
        try:
            ...
            self.conn.commit()
            messagebox.showinfo("Éxito", "Mensaje respondido correctamente.")
        except mysql.connector.Error as err:
            messagebox.showerror("Error", f"Error al responder mensaje: {err}")
