...
class MensajeApp:
    ...
    def responder_mensaje(self):
        ...
        if mensaje:
            ...
            fecha_gestion = datetime.now().strftime("%Y-%m-%d %H:%M:%S")

            tk.Label(responder_window, text="Fecha de Gestión:").grid(row=6, column=0, pady=5)
            tk.Label(responder_window, text=fecha_gestion).grid(row=6, column=1, pady=5)
            ...
            def enviar_respuesta():
                ...
                try:
                    self.conectar_base_datos()  # Asegúrate de estar conectado a la base de datos
                    ...
                    messagebox.showerror("Error", f"Error al enviar respuesta: {err}")
                finally:
                    self.cursor.close()
                    self.conn.close()
