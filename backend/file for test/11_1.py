...
class Mensaje:
    def __init__(self, host, user, password, database):
        ...
        with self.conn.cursor(dictionary=True) as cursor:
            ...
            cursor.execute('''CREATE TABLE IF NOT EXISTS connect_box (
    ...
    def eliminar_mensaje(self, id):
        with self.conn.cursor() as cursor:
            cursor.execute(f"DELETE FROM connect_box WHERE id = {id}")
        self.conn.commit()
        return cursor.rowcount > 0
...
# Create the object
mensaje = Mensaje(host='localhost', user='root', password='', database='bakers_box')
